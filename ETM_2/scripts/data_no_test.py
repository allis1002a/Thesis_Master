...
patents_source = pd.read_csv('patents_source_brief.csv')
all_patents = list(patents_source['brief'])
docs = all_patents
...
idx_permute = range(num_docs)
print(type(idx_permute))
...
def remove_empty(in_docs):
    ...
    for i, doc in enumerate(in_docs):
        if doc!=[]:
            ...
            removed_doc.append(i)
            
    with open("removed_doc.txt", "w") as txt_file:
        for line in removed_doc:
...
path_save = './no_test_min_df_' + str(min_df) + '/'
